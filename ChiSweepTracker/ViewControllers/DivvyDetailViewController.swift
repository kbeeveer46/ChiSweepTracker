...
class DivvyDetailViewController: UIViewController, MKMapViewDelegate {
	...
	func populateStationLabels() {
		...
		// Get JSON URL
        let url = URL(string: self.common.defaults.divvyJSONUrl())
		...
		URLSession.shared.dataTask(with:url!, completionHandler: {(results, response, error) in
			...
			do {
                ...
                let lastUpdated = json?[self.common.defaults.divvyJSONLastUpdatedTitle()] as? Double ?? 0.0
				...
				let data = json?["\(self.common.defaults.divvyJSONDataTitle())"] as? [String: Any] ?? [:]
				
				// Get station element from data
				let stations = data["\(self.common.defaults.divvyJSONStationsTitle())"] as? [[String: Any]] ?? []
				...
				for item in stations {
					...
					let id = item["\(self.common.defaults.divvyJSONIdTitle())"] as? String ?? ""
